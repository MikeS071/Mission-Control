...
## Objective
Visual indicator on kanban cards that haven't been updated recently (stale detection).

## Requirements
1. `src/lib/kanban/staleDetector.ts`:
   - `isStale(task: { updatedAt: Date }, thresholdDays: number): boolean`
   - `getStaleLevel(task: { updatedAt: Date }): 'fresh' | 'aging' | 'stale'`
   - fresh: <3 days, aging: 3-7 days, stale: >7 days (configurable thresholds)
2. UI changes in kanban board components:
   - Add subtle visual indicator (border color or badge) for aging/stale cards
   - Tooltip showing "Last updated X days ago"
3. Tests in `src/__tests__/lib/stale-detector.test.ts`
4. Component tests for the stale indicator rendering

## Dependencies
- mc-22 (kanban bot triggers — provides event data)
