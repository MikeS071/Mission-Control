...
## Objective
Admin panel showing database statistics — table sizes, row counts, connection pool.

## Requirements
1. `src/lib/admin/dbStats.ts`:
   - `getTableStats(): Promise<TableStat[]>` — table name, row count, size estimate
   - `getConnectionPoolStats(): Promise<PoolStats>` — active, idle, waiting, total
   - Uses `pg_stat_user_tables` and `pg_relation_size` for Postgres
2. `src/app/api/admin/system/db/route.ts`:
   - `GET` — returns table stats + pool stats
3. `src/app/admin/system/db/page.tsx`:
   - Table listing all user tables with row count and size
   - Connection pool status card
4. Tests in `src/__tests__/api/db-stats.test.ts`

## Dependencies
- mc-31 (system health — shares admin/system layout)
