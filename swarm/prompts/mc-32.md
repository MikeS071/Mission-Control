...
## Objective
Admin page to view recent application logs with filtering.

## Requirements
1. `src/app/admin/system/logs/page.tsx`:
   - Table: timestamp, level (info/warn/error), message, source
   - Filter by level, search by message text
   - Last 200 entries, newest first
   - Auto-refresh toggle
2. `src/app/api/admin/system/logs/route.ts`:
   - `GET ?level=&q=&limit=` — query logs
3. `src/lib/admin/appLogger.ts`:
   - `AppLogEntry { timestamp, level, message, source, metadata? }`
   - `getRecentLogs(opts: { level?, query?, limit? }): Promise<AppLogEntry[]>`
   - Store in DB table: app_logs (id, timestamp, level, message, source, metadata_json)
4. Tests in `src/__tests__/api/system-logs.test.ts`

## Dependencies
- mc-01 (admin layout exists)
