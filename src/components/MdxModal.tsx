...
export function MdxModal({ open, onOpenChange, title, initialContent, onSave, staleCheckUrl, loadedVersion }: MdxModalProps) {
  ...
  }, [open, initialContent, loadedVersion]);
    ...
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="bg-gray-950 border-gray-800 text-white sm:max-w-3xl max-h-[85vh] flex flex-col">
          <DialogHeader>
            <DialogTitle>{title ?? 'Edit PRD'}</DialogTitle>
            <DialogDescription>Edit the markdown content below.</DialogDescription>
          </DialogHeader>
