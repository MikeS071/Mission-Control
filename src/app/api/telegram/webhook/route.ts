...
export async function POST(req: NextRequest) {
  try {
    ...
    // Bridge disabled: still ACK 200 so Telegram doesn't retry.
    ...
    // Persist enough payload so the retry worker can process even if the request ends.
    ...
    // Ignore non-message updates / missing payload.
    if (!chatId || !telegramUserId || !telegramMessageId) return NextResponse.json({ ok: true });
    ...
    if (text.toLowerCase().startsWith('/start')) {
      ...
      await db.update(telegramLinkTokens).set({ consumedAt: new Date() }).where(eq(telegramLinkTokens.token, token));
    ...
    // UX: show typing immediately; the retry worker will maintain typing during processing.
    void telegramSendChatAction(chatId, 'typing');

    // Reliability: enqueue the work in DB and let the retry worker process it.
    await db
      .update(telegramUpdates)
      .set({
        tenantId: link.tenantId,
