  ...
  return (
    <section className="space-y-6 text-gray-100">
      <header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        ...
        <Link
          href="/admin/content/new?mode=create"
          className="inline-flex items-center justify-center rounded-md border border-sky-600 bg-sky-600/20 px-4 py-2 text-sm font-medium text-sky-200 transition hover:bg-sky-600/30"
        >
          New Content
        </Link>
      </header>
                ...
                items.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-800/40">
                    <td className="px-4 py-3 font-medium text-gray-100">
                      <Link href={`/admin/content/${item.id}`} className="text-sky-300 hover:text-sky-200 hover:underline">
                        {item.title}
                      </Link>
                    </td>
